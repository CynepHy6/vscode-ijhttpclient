...
При запуске запросов из расширения рядом с `.http`-файлом также может появляться локальный каталог `.response/` с сохранёнными полными дампами `*.response`, а под выполненным запросом будут добавляться history-комментарии вида `# <> ./.response/...`.
