...
export const MetadataLineRegex: RegExp = /^\s*(?:#|\/{2})\s*@([\w-]+)(?:\s*(?:=)?\s*(.*?))?\s*$/;
export const FileVariableDefinitionRegex: RegExp = /^\s*@([^\s=]+)\s*=\s*(.*?)\s*$/;
export const ResponseStatusLineRegex: RegExp = /^\s*HTTP\/[\d.]+\s+\d{3}\b/;
export const RequestLineRegex: RegExp = /^(?:(GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS|CONNECT|TRACE|LOCK|UNLOCK|PROPFIND|PROPPATCH|COPY|MOVE|MKCOL|MKCALENDAR|ACL|SEARCH|BIND|REBIND|UNBIND|LINK|UNLINK|MERGE|PURGE|REPORT|MKACTIVITY|CHECKOUT|MSEARCH|NOTIFY|SUBSCRIBE|UNSUBSCRIBE)\s+)?(?:https?:\/\/|wss?:\/\/|{{|[A-Za-z0-9._~!$&'()*+,;=:@%\/?-]).*$/i;
